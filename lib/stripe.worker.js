...
const jsonSuccess = (data, extra = {}) => {
  return withCors(
    new Response(JSON.stringify({ success: true, ...data, ...extra }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    })
  );
...
router.post('/api/projects/save', async ({ request, user }) => {
  try {
    ...
    if (project.renderedImage && !project.renderedImage.includes('watermark')) {
      ...
      if (!renderValidation.allowed) return jsonError(403, renderValidation.error);
    }
    ...
    if (project.renderedImage && !renderedImageTooLarge) payload.renderedImage = project.renderedImage;
    ...
    if (e.message && e.message.includes('quota')) errorMessage = 'Storage quota exceeded. Please free up space.';
    else if (e.message && e.message.includes('size')) errorMessage = 'Project data too large. Please reduce image size.';
...
router.get('/api/projects/get', async ({ request, user }) => {
  try {
    ...
    if (!isOwner && !project.isPublic && !isPublicAccess) return jsonError(403, 'Access denied');
...
router.post('/api/projects/share', async ({ request, user }) => {
  try {
    ...
    if (isPublic && !shareToken) shareToken = generateShareToken();
...
router.get('/api/projects/gallery', async ({ request, user }) => {
  try {
    ...
    if (search) {
      ...
      projects = projects.filter((p) => p.name?.toLowerCase().includes(searchLower) || p.id.toLowerCase().includes(searchLower));
    }

    if (sortBy === 'recent') projects.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
    else if (sortBy === 'popular') projects.sort((a, b) => (b.viewCount || 0) - (a.viewCount || 0));
    else if (sortBy === 'mostLiked') projects.sort((a, b) => (b.likeCount || 0) - (a.likeCount || 0));
...
router.post('/api/create-checkout-session', async (context) => {
  try {
    if (!STRIPE_SECRET_KEY) return jsonError(500, 'Payment system not configured');
    ...
    if (!finalUserId) return jsonError(401, 'Authentication failed - no user ID found');

    const { priceId, email, planType, successUrl, cancelUrl } = body;
    if (!priceId || !finalUserId || !email) return jsonError(400, 'Missing required parameters');
    ...
    if (!stripeResponse.ok) return jsonError(500, `Stripe error: ${stripeData.error?.message || 'Unknown error'}`);
...
router.get('/api/verify-payment', async (context) => {
  try {
    if (!STRIPE_SECRET_KEY) return jsonError(500, 'Payment verification not configured');

    let userPuter = null;
    if (context.user && context.user.puter) userPuter = context.user.puter;
    else if (context.puter) userPuter = context.puter;
    ...
    if (!stripeResponse.ok) return jsonError(500, `Stripe error: ${session.error?.message || 'Unknown error'}`);
...
router.post('/api/analytics/track', async ({ request, user }) => {
  try {
    ...
    if (!event || !event.id || !event.name) return jsonError(400, 'Invalid event data');
...
router.post('/api/analytics/batch', async ({ request, user }) => {
  try {
    ...
    if (!events || !Array.isArray(events) || events.length === 0) return jsonError(400, 'Invalid batch events data');
...
router.get('/api/analytics/stats', async ({ request, user }) => {
  try {
    ...
    for (const { value: event } of allKeys) {
      ...
      if (event.name === 'feature_used' && event.properties?.feature) features[event.properties.feature] = (features[event.properties.feature] || 0) + 1;
      if (event.name === 'conversion' && event.properties?.step) conversions[event.properties.step] = (conversions[event.properties.step] || 0) + 1;
    }

    const usersByPlan = { free: 0, pro: 0 };
    for (const uid of users) {
      const usage = await userPuter.kv.get(`roomify_usage_${uid}`);
      if (usage?.isPremium) usersByPlan.pro++;
...
// Email
router.options('/api/email/subscribers', () => new Response(null, { status: 204, headers: corsHeaders }));
router.options('/api/email/campaigns', () => new Response(null, { status: 204, headers: corsHeaders }));
router.options('/api/email/subscribe', () => new Response(null, { status: 204, headers: corsHeaders }));
router.options('/api/email/unsubscribe', () => new Response(null, { status: 204, headers: corsHeaders }));
router.options('/api/email/campaigns/create', () => new Response(null, { status: 204, headers: corsHeaders }));
router.options('/api/email/campaigns/send', () => new Response(null, { status: 204, headers: corsHeaders }));
router.options('/api/email/track', () => new Response(null, { status: 204, headers: corsHeaders }));
...
router.post('/api/email/subscribe', async ({ request, user }) => {
  try {
    ...
    if (subscriber && subscriber.status === 'active') return jsonSuccess({ alreadySubscribed: true, email });
...
router.post('/api/email/track', async ({ request, user }) => {
  try {
    ...
    if (!campaignId || !email || !eventType) return jsonError(400, 'Campaign ID, email, and eventType required');
